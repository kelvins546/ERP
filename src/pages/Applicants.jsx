...
function ApplicantModal({ applicant, onClose, onSaved }) {
  ...
  const save = async () => {
    setSaving(true);
    try {
      // Clean the payload to match what the SQL table actually expects
      // Note: If you want email, phone, etc to save, you must add those columns to Supabase
      const payload = {
          ...
          {stages.map((stage) => (
            <div key={stage} className="flex-shrink-0 w-64">
              <div className="flex items-center justify-between mb-3">
                <span
                  className={`text-xs font-semibold px-2 py-1 rounded-full capitalize ${stageColors[stage] || "bg-slate-100"}`}
                >
                  {stage}
                </span>
              ...
              <div className="space-y-2">
                {byStage(stage).map((a) => (
                  <div
                    key={a.id}
                    className="bg-white rounded-xl border border-slate-200 p-3 shadow-sm cursor-pointer hover:shadow-md transition-shadow"
                    onClick={() => {
                      setEditApplicant(a);
                      setShowModal(true);
                    }}
                  >
                    ...
                    {a.email && (
                      <p className="text-xs text-slate-400 mt-1">{a.email}</p>
                    )}
                    {a.source && (
                    ...
                    <div className="flex gap-1 mt-2">
                      {stages.indexOf(a.status) > 0 && (
                        <button
                          ...
                          className="text-xs text-slate-400 hover:text-slate-700 px-1.5 py-0.5 rounded border border-slate-200 hover:bg-slate-50"
                        >
                          ← Back
                        </button>
                      )}
                      {stages.indexOf(a.status) < stages.length - 1 && (
                        <button
                          ...
                          className="text-xs text-blue-600 hover:text-blue-700 px-1.5 py-0.5 rounded border border-blue-200 hover:bg-blue-50 ml-auto"
                        >
                          Next →
                        </button>
                      )}
